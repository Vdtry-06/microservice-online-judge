...
app.get("/api/metrics", async (req, res) => {
  try {
    const [problemsHealth, submissionsHealth, usersHealth] =
    ...
    res.json({
      ...
      problemService:
        problemsHealth.status === "fulfilled"
          ? problemsHealth.value.data
          : { error: "unavailable" },
      submissionService:
        submissionsHealth.status === "fulfilled"
          ? submissionsHealth.value.data
          : { error: "unavailable" },
      userService:
        usersHealth.status === "fulfilled"
          ? usersHealth.value.data
          : { error: "unavailable" },
      timestamp: new Date().toISOString(),
    });
  } catch (error) {

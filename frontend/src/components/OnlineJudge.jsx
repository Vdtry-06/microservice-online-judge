...
function ProblemPage({ problem, onBack }) {
  ...
  const runTest = () => {
    ...
    try {
      ...
      setTestRunResult({
        ...
        passed: JSON.stringify(output) === JSON.stringify(JSON.parse(testCase.output)),
      });
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // FIXED POLLING - Use async/await properly
      const poll = async () => {
        if (signal.aborted) {
        ...
        try {
          ...
          // CHECK IF DONE - Stop immediately
          if (res.status === "accepted" || res.status === "wrong_answer" || res.status === "failed") {
            if (debugMode)
              console.log("Complete in", attempts, "polls:", res.status);
            setResult(res);
            setSubmitting(false);
            return; // ← CRITICAL: Exit here, no more recursion
          }

          // Still processing - continue after 1s
          if (debugMode) console.log("Still", res.status, "- waiting 1s");

          // FIX: Use setTimeout with Promise to control recursion
          await new Promise((resolve) => setTimeout(resolve, 1000));

          // Check abort again before recursing
          if (!signal.aborted) {
            await poll(); // Recursive call
          }
        } catch (error) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-slate-800/50 backdrop-blur border border-purple-500/20 rounded-xl p-6 overflow-y-auto max-h-[calc(100vh-200px)]">
        ...
        <span
          className={`px-3 py-1 rounded-full text-sm font-medium border inline-block mb-6 ${

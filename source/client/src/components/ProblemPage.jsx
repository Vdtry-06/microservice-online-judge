...
export default function ProblemPage({ onBack }) {
  ...
  // Show error if problem not found
  if (!problem) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          <XCircle className="w-16 h-16 text-red-400 mx-auto mb-4" />
          <p className="text-white text-xl mb-2">Problem not found</p>
          <button
            onClick={onBack}
      ...
      const poll = async () => {
        ...
        try {
          ...
          if (
            ...
            // if (debugMode) console.log("Complete in", attempts, "polls:", res.status);
            setResult(res);
            setSubmitting(false);
              ...
              if (refreshUser) {
                ...
                window.dispatchEvent(
                  new CustomEvent("user:updated", {
                    detail: { problemId: problem.id, result: res },
                  })
                );
              }
            }

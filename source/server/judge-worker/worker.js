...
function executeJava(code, testCases) {
  ...
  const GSON_JAR = process.env.GSON_JAR || "/app/gson-2.8.9.jar";
...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        // Parse input - could be array, string, number, or object
        Object input = gson.fromJson(inputLine, Object.class);
...
`;
    ...
    execSync(`javac -cp ${GSON_JAR} ${javaFile}`, {
      cwd: tempDir,
      timeout: 10000,
      encoding: "utf-8",
    });
    ...
    for (const testCase of testCases) {
      try {
        ...
        const outputStr = execSync(`java -cp ${GSON_JAR}${separator}. Main`, {
          cwd: tempDir,
          input,
          timeout: 5000,
          encoding: "utf-8",
  ...
  try {
    ...
    if (allPassed && userId && problemId) {
      try {
        ...
        console.log(
          `[${WORKER_ID}] Updated solved problems for user ${userId}`
        );
      } catch (err) {
